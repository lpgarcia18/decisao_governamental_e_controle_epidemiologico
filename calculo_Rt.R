...
# Total de infectados por data de início de sintomas. Dados extraídos de https://app.powerbi.com/view?r=eyJrIjoiNDMyMDhkMWItZTI3NC00ZTkzLWJiNTEtOWE1YWQxZjg4MjI2IiwidCI6ImExN2QwM2ZjLTRiYWMtNGI2OC1iZDY4LWUzOTYzYTJlYzRlNiJ9
#Extração realizada dia 25/08/2020
covid <- read_delim("dados/boavista_covid_dados_abertos.csv", 
    ";", escape_double = FALSE, trim_ws = TRUE)

covid$CASOS <- 1
covid$INICIO_SINTOMAS <- as.Date(covid$data_inicio_sintomas)
covid <- covid %>% 
...
for(i in seq_along(covid$INICIO_SINTOMAS)){
	...
	incidencia <- covid[i:(15+i),] #Utilizando dados de um mês
	incidencia <- na.omit(incidencia)

	# Truncagem à esquerda
	trunc <- 2
	
	Icovid <- incidencia$CASOS
	gencovid <- EpiEstim::discr_si(c(0:max(incidencia$CASOS)), mu = 4.8, sigma = 2.3)
...
anterior_publicacao <- (as.Date(decretos$PUBLICACAO) - 1) %>% as.character() #Selecionando Rt anterior a data da publicação do decreto
...
decr <- subset(decr, decr$PUBLICACAO <= as.Date("2020-07-15"))

decr_fim <- na.omit(decr)
#Escrevendo o resultado
write.csv(decr_fim, "resultados/resultado.csv", row.names = F)
...
decr$PUBLICACAO <- as.Date(decr$PUBLICACAO) 
ggplot(decr, aes(PUBLICACAO,INDICE, group = 1))+
	geom_line()+
	ylim(-1,1)+
	theme_bw()

covid_graf <- covid
covid_graf <- subset(covid_graf, covid_graf$INICIO_SINTOMAS <= as.Date("2020-07-15"))
names(covid_graf)[1] <- "DATA"
ggplot(covid_graf, aes(DATA))+
       	geom_smooth(aes(y = CASOS))+
	geom_vline(xintercept = as.Date("2020-03-16"), size =1.3)+
	geom_vline(xintercept = as.Date("2020-03-17"), size =1.3)+
	geom_vline(xintercept = as.Date("2020-03-19"), size =1.3)+
	geom_vline(xintercept = as.Date("2020-03-23"), linetype='dashed')+
	geom_vline(xintercept = as.Date("2020-04-02"), size =1.3)+
	geom_vline(xintercept = as.Date("2020-04-11"), linetype='dashed')+
	geom_vline(xintercept = as.Date("2020-04-17"), linetype='dashed')+
	geom_vline(xintercept = as.Date("2020-04-30"), linetype='dashed')+
	geom_vline(xintercept = as.Date("2020-06-01"), linetype='dashed')+
	geom_vline(xintercept = as.Date("2020-06-05"), linetype='dashed')+
	geom_vline(xintercept = as.Date("2020-07-13"), linetype='dashed')+
	theme_bw()
